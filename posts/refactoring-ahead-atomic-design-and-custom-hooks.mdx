...
인스타그램 홍보와 외부 홍보 부스를 마련할 기회가 몇 번 더 있었기 때문에, 추가 유입을 위해서는 사용성 피드백에 즉각 대응해 최대한 빠르게 수정 사항을 적용해야 했습니다.
...
최대한 재사용되는 컴포넌트를 작성하기 위해서 해당 컴포넌트가 적용되는 모든 상황을 고려해 기본 인터페이스를 추상화하려고 고민했습니다.
...
`FeedItem`을 내부에 가지고 있지만 반복적으로 가지고 있지는 않기 때문에, List 같은 컴포넌트와는 구분되어야 한다고 생각했어요.
...
최상위 인터페이스인 page 하위에 template을 분리해 컴포넌트를 배치하는 단계는 주입되는 데이터에 대해 다양하게 존재할 수 있는 케이스를 고려하고
...
설정 메뉴 리스트를 제공하기 위해 `organisms/MenuList`에 필요한 데이터를 주입하는 것으로 마무리할 수 있었어요.
...
이를 재사용하기 위해서 분리하는 것이 좋다는 것은 더 받아들이기 쉬운 맥락이기도 하고요.
