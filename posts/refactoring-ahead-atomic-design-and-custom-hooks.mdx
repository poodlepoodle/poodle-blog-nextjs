...
description: '아토믹 디자인과 커스텀 훅을 활용해 컴포넌트 구조 리팩토링하기'
...
프론트엔드 개발로 참여해 약 3개월 동안의 시간 동안 MVP 기능 구현을 마쳤고 3월 초에 서비스를 런칭했습니다.

![](/blog/posts/refactoring-ahead-atomic-design-and-custom-hooks/seasoning-thumbnail.png?width=450)
...
그 밖에 API 요청이나 라우팅처럼 공통 비즈니스 로직에 관한 부분은 `src/utils` 내에서 관리하도록 했고요.

### 기존 구조의 문제점

**1. 재사용되지 않는 컴포넌트와 기능 추가에 따르는 리소스 낭비**

![](/blog/posts/refactoring-ahead-atomic-design-and-custom-hooks/need-more-things.jpg?width=300)
...
![](/blog/posts/refactoring-ahead-atomic-design-and-custom-hooks/content-editor-component-1.jpg)
...
초기 데이터로 제공하는지의 여부를 제외하면, 나머지 FormData 처리와 작성 중인 게시글 컨텐츠를 편집하는 로직은 두 페이지가 거의 같은 코드를 공유하고 있었어요.

![](/blog/posts/refactoring-ahead-atomic-design-and-custom-hooks/content-editor-component-2.jpg)
...
다시 말해 “컴포넌트의 재사용성”에 관한 일관되고 보다 명확한 기준이 필요했습니다.
...
친구 피드 페이지(`/feed`)나, 알림 페이지(`/notification`)등 일부 페이지에서는 API 구조가 페이지네이션 구조로 설계되어 있습니다.
...
![](/blog/posts/refactoring-ahead-atomic-design-and-custom-hooks/atomic-design-brad-frost.png?width=550)
...
또한 프로젝트를 개발하며 가장 아쉬웠던 점이 스토리북, Jest 등을 이용한 단위 테스트를 적용하지 않은 것이었는데,
...
![](/blog/posts/refactoring-ahead-atomic-design-and-custom-hooks/atom-example.jpg?width=450)
...
초기에는 `Row`, `Column` 등 자주 사용하는 레이아웃 스타일도 atom으로 분리했으나,
...
재사용되는 컴포넌트를 설계하는 여러 사례를 참고하며, 컴포넌트에 필요한 기본 인터페이스는 어느 정도 정해 놓고
컴포넌트를 사용하는 시점에 필요한 스타일을 추가로 주입하도록 하는 패턴을 봤는데요,
이는 납득할 수 있는 좋은 방법이라고 느꼈으나 어디까지 주입하게 할 지에 대해서는 시즈닝의 상황에 집중해 조금 더 고민할 필요가 있다고 생각했습니다.
따라서 각 atom 컴포넌트가 사용될 수 있는 여러 상황을 충분히 고려하고, 자주 변경되는 속성만을 기본 props로 정의했습니다.
...
- 컴포넌트의 UI에 대한 선언 부분을 분리
  - `style.js`: 컴포넌트의 스타일 요소
  - `index.jsx`: 컴포넌트의 몸체
- JSX 상에서 컴포넌트와 Styled 요소를 구분하기 쉬움
...
![](/blog/posts/refactoring-ahead-atomic-design-and-custom-hooks/molecule-example.jpg?width=670)
...
![](/blog/posts/refactoring-ahead-atomic-design-and-custom-hooks/differences-between-molecule-and-organism-example-1-tabbar.jpg?width=700)
...
![](/blog/posts/refactoring-ahead-atomic-design-and-custom-hooks/differences-between-molecule-and-organism-example-2-feeditem.jpg)
...
![](/blog/posts/refactoring-ahead-atomic-design-and-custom-hooks/differences-between-molecule-and-organism-example-3-menulist.jpg)
...
“반복적인 내부 컴포넌트를 가지고 있는 경우”로만 한정할 수 있어 판단이 좀 더 쉬워졌습니다.
...
![](/blog/posts/refactoring-ahead-atomic-design-and-custom-hooks/component-naming-example-carousel.jpg)
...
![](/blog/posts/refactoring-ahead-atomic-design-and-custom-hooks/template-page-example.jpg)

- 레이아웃 배치만을 위한 단위
  - 하나의 page는 중첩 라우팅을 기준으로 최소 하나의 template에 1대1 대응
- 상태 관리 등 비즈니스 로직은 일괄 제외

Template에서는 페이지의 내용을 그리기 위해 필요한 레이아웃과 스타일 등 시각적인 요소들을 배치하고 조립했습니다.
이를 위해 필요한 atom과 molecule, organism을 호출하고 레이아웃 스타일을 배치했어요.

**Template**과 page는 특정 페이지에 해당하는 내용만을 표현하기 위해 사용되는 단위이므로 1대 1로 대응된다는 특성이 있습니다.
단, `/home?category=year`처럼 한 페이지 내에서 Search Parameter에 따라 다르게 렌더링되는 영역이 존재하는 경우에는 template을 중첩해 표현했어요.

### 추가: HoC 패턴으로 공통 바깥 레이아웃을 분리

![](/blog/posts/refactoring-ahead-atomic-design-and-custom-hooks/template-using-hoc-pattern.jpg)
...
*(Context에 대해서는 아래에서 좀 더 자세히 말씀드리려고 합니다.)*
...
![](/blog/posts/refactoring-ahead-atomic-design-and-custom-hooks/separation-of-concerns-example-hoc.jpg?width=550)
...
![](/blog/posts/refactoring-ahead-atomic-design-and-custom-hooks/separation-of-concerns-example-custom-hooks.jpg)
...
로직을 재사용하는 과정에 커스텀 훅을 적극적으로 활용해 보면 좋겠다고 판단한 이유입니다.

### 아토믹 디자인의 Page-level Context를 커스텀 훅으로 제공하기

시즈닝에서는 React-router의 API Loader를 활용해 페이지를 그리는 데 필요한 정보를 fetch하고 나서 페이지가 렌더링되도록 했어요.
그리고 page에서 필요한 상태와 로직을 Context Provider로 제공해 template에서 이를 받아 하위 컴포넌트들로 주입했습니다.
그 중간 단계에서, 각 페이지에 해당하는 context를 통해 상태와 로직을 세팅하는 부분을 커스텀 훅으로 분리했습니다.
...
Page에서 직접 로직을 선언하고 제공까지 하도록 할 수도 있었지만, 전역 context를 분리하도록 함으로써 page가 전역 context의 provider 역할만 수행하도록 하고 싶었어요.
결과적으로 어떤 페이지 내의 기능과 뷰를 각각 context와 template에 위임하고 이를 page에서 결합하는 형태로 책임을 분리했습니다.
...
모든 페이지에 대해서 전역 Context에 책임을 위임하지는 않았습니다.
예시로 `NoticePage` 같은 경우 상태 관리나 함수의 필요 없이
API Loader에서 fetch한 공지사항 데이터만 template에 주입하면 됐기 때문에 그냥 template으로 props를 전달하도록 했습니다.
...
│   ├── templates
│   └── hoc
├── pages       // 페이지 컴포넌트: 라우팅 기준으로 분류
│   ├── feed
│   ├── home
│   └── ...
├── contexts    // 페이지 컨텍스트: 페이지 기준으로 분류
...
많은 적용 사례에서 아토믹 디자인은 **Silver Bullet** _(: 모든 상황을 한 번에 해결하는 완벽한 해결책)_ 이 아님을 강조했는데요,
...
React에서 전역 상태를 다룰 수 있는 도구인 Context API를 제공함에도 불구하고 Redux, Recoil 등 상태 관리 라이브러리를 사용하는 경우가 대다수인 이유는
상단의 props의 변경에 의해 형제 이하 요소의 Re-rendering이 발생하는 현상을 핸들링하기 어려운 경우가 발생한다는 점입니다.
(물론 이것뿐은 아닙니다.)
Children props를 활용해 전달함으로써 Re-rendering을 방지할 방법도 있지만
서비스의 기능이 복잡해질수록 고려해야 하는 것들 또한 복잡해질 것입니다.

시즈닝에서 페이지의 전역 상태를 Context API로 제공하도록 한 부분은 필요한 러닝 커브가 거의 없다는 점도 있었지만
사실 가장 큰 이유는 상태 관리 라이브러리를 사용해야 하는 상황을 실제로 느껴 보고 싶었기 때문이었습니다.
상태 관리 라이브러리를 사용해야 한다면 Context API가 해결해 주지 못하는 문제가 무엇인지를 직접 느껴 보고 그 시점부터 도입을 고려해야 한다고 생각했습니다.
특히 최근에는 많은 곳에서 React Query를 사용해 server state와 client state를 관리한다고 느꼈는데,
어떤 필요성에 의해 서비스들이 이런 기술을 사용하게 되었는지의 흐름을 이해하는 게 중요하다고 생각했습니다.

이번 리팩토링에서 아토믹 디자인을 적용하며 하위 컴포넌트들을 상위 컴포넌트가 감싸는 형태로 화면을 구성하다 보니
Re-rendering 문제가 보다 와닿았습니다. 또 페이지 단위로 전역 context를 구성했지만
`~List` - `~ListItem`으로 감싸질 것이 분명한 컴포넌트끼리는 자체적인 context를 가지도록 해도 좋겠다고도 생각했어요.
Context API로 구현할 수 있었던 부분들이지만, 더 나은 구조를 위해 상태 관리 라이브러리들이 제공하는 기능을
활용하는 것도 충분히 시도해 볼 만한 방향이라는 것을 직접 느꼈습니다.
...
이 과정에서 타입스크립트의 필요성을 느꼈습니다.
타입스크립트를 사용하면 props, context로 안전한 형태의 값만 전달되도록 보장할 수 있으며,
에디터에서 제공하는 기능을 통해 편리하게 전달되는 값의 인터페이스를 들여다볼 수 있으니까요.
어떤 블로그 포스트를 지나치다가 읽었던, "가장 안전한 경우는 코드를 작성하는 레벨에서 알 수 없는 값이 전달되는 경우의 가능성 자체를 없애버리는 것"이라는 표현을 인용하고 싶습니다.

이전부터 타입스크립트의 장점을 알고도 러닝 커브 때문에 쉽사리 프로젝트에 적용하지는 못했습니다.
하지만 iOS 개발을 접하고 또 다른 정적 언어인 Swift에서 타입 가드를 통해 엄격하게 값을 처리하는 것을 보며
타입스크립트에서 활용하는 타입 관련 패턴을 좀 더 이해하게 되었어요.
특히 현재 프로젝트에 적용해야 할 필요성이 많이 드러난 만큼 꼭 적용해야 한다고 다시 한번 느꼈습니다.
...
이번 리팩토링에서는 **Storybook**까지 직접 적용하지는 않고 적용하기 좋은 형태를 갖추는 데에 그쳤지만
아토믹 디자인을 적용한 대부분의 사례를 참고하며 Storybook을 활용해 컴포넌트 개발을 진행하는 것의 장점을 느꼈습니다.
어쩌면 초기부터 도입하지 못한 게 가장 아쉬워지는 부분이었습니다.
비슷한 맥락에서 테스트 코드의 필요성 또한 체감했고요.

---

이번 작업을 진행하면서 좋은 프로젝트의 구조에 대해 여러 사례를 접하고 고민할 수 있는 계기가 됐습니다.
이것저것 적용해 보면 좋겠는 흥미로운 기술도 많았지만, 본질이 흐려지지 않도록
...
- 아토믹 디자인에 관해
  - [Atomic Design Methodology | Brad Frost (영어 원문)](https://atomicdesign.bradfrost.com/chapter-2/#the-part-and-the-whole)
...
- 나머지 부분들에 대해
  ...
  - [HTML Semantic Elements | W3Schools (영어 원문)](https://www.w3schools.com/html/html5_semantic_elements.asp)
