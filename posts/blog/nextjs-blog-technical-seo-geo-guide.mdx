...
앞서 다룬 `BlogPosting` · `CollectionPage` 스키마를 예시로 들면, `author` 값에는
...
추가로, TypeScript 환경에서 스키마를 작성한다면 [schema-dts](https://www.npmjs.com/package/schema-dts) 패키지를 사용하는 것이 좋습니다.
패키지 내에는 schema.org 기반의 JSON-LD 스키마들이 타입으로 정의되어 있어,
스키마 객체에 타입을 명시하는 것만으로 필드 누락이나 형식이 다른 값이 들어가는 것을 컴파일 단계에서 방지합니다.
...
const LOGO: ImageObject = {
  ...
  // width: 512,    TypeError: 'number' 형식은 'SchemaValue<string | IdReference ...
  width: '512',
  height: '512',
};
