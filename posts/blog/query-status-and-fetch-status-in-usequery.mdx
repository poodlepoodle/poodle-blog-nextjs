...
다만, 저의 경우에는 처음 봤을 때 이름만으로 각 플래그의 역할을 구분하기 어려웠습니다.
...
`status`나 `fetchStatus` Union 타입의 각 값에 `isPending`, `isFetching`과 같은 상태 플래그가 대응하는 구조이기 때문입니다.
...
각 상태 플래그는 `status` Union 타입의 하나의 값에 대응하므로 상호 배타성을 가집니다.
...
Query Status와 마찬가지로 `fetchStatus`의 상태 플래그 또한 상호 배타성을 가집니다.
...
Query Status 및 Fetch Status에 직접 포함된 상태 플래그 외에도
여러 상태 플래그를 조합해서 다양한 상황을 표현하는 파생 플래그들이 함께 제공되고 있습니다.
...
결과가 성공이든 에러든 요청 시도가 한 번이라도 수행 완료되었음을 의미하는 플래그들입니다.
...
![Case 2-a. Succeeded Fetching Data](/posts/query-status-and-fetch-status-in-usequery/case-2-a-succeeded-fetching-data.webp)
...
![Case 2-b. Paused Until Network Connected](/posts/query-status-and-fetch-status-in-usequery/case-2-b-paused-until-network-connected.webp)
...
![Case 2-c. Error Occurred While Fetching](/posts/query-status-and-fetch-status-in-usequery/case-2-c-error-occurred-while-fetching.webp)
...
추가로, 타입스크립트 4.6버전 이후로 타입 추론 지원이 강화되어 `isFetching`과 같은 파생 플래그도
...
![Comparison of useQuery Status Changes](/posts/query-status-and-fetch-status-in-usequery/comparison-of-usequery-status-changes.webp)
