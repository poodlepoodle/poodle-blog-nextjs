...
React에서 스크롤 이벤트를 이용해 스크롤에 따라 변경되는 정보를 처리한 후
...
> 하나의 예시로, 사이드에서 나타나는 내비게이션은 사라질 때 애니메이션 타이밍을 `ease-in`으로 적용해
...
방문자에게 어떤 경험을 제공할 수 있을지 직접 결정할 수 있다는 부분이 매우 큰 장점이라고 느꼈어요.
