...
이후 재요청이 발생되는 시점에야 `isFetchedAfterMount: true`가 됩니다.
...
`isLoading`이나 `isInitialLoading`을 활용해 더 간결하게 나타낼 수 있습니다.
...
> useQuery로 네트워크를 필요로 하지 않는 비동기 요청(ex: 무거운 Web Worker 요청 등)을 수행할 때는
네트워크 상태에 의해 해당 요청이 중단되지 않도록 v4부터 도입된
[`networkMode`](https://tanstack.com/query/latest/docs/framework/react/guides/network-mode) 옵션을 설정할 수 있습니다.
...
useQuery는 요청이 실패한 경우 기본적으로 3번의 재시도를 수행하며, 모든 재시도가 실패한 경우에 Query Status를 `error`로 전이합니다.
...
이때 데이터가 존재하므로 Query Status는 `pending`이지만, Query Status는 `fetching`이 됩니다.

이 상태에서는 기존에 요청에 성공한 데이터가 존재하므로, 데이터를 가리지 않는 별도의 영역에서 로딩 인디케이터를 표시하는 것이 적절합니다.
...
_- TkDodo의 "Status Checks in React Query" 포스트 원문 중_
